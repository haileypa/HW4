...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/haileypaHW4"
...
class TweetForm(FlaskForm):
    text = StringField("Whats Happening?", validators=[Required()])
    username = StringField("Username:",validators=[Required()])
    hashtags = StringField("Enter your hashtags separated by commas:", validators
        =[Required()])
...
def get_or_create_tweet(db_session, tweet_text, username):
    tweet = db_session.query(Tweet).filter_by(text=tweet_text, user_id=get_or_create_user(db_session, username).id).first()
    if tweet:
        return tweet
    else:
